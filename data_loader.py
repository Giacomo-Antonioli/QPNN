...
#     print('tr_x',len(tr_x),tr_x[0:10], 'tr_y', len(tr_y),tr_y[0:10], 'te_x',len(te_x),te_x[0:10],'te_y',len(te_y),te_y[0:10])
    ...
    if zipped == True:
        ...
        tr_x=np.reshape(tr_x,(-1,dim_scale))
        te_x=np.reshape(te_x,(-1,dim_scale))
        va_x=np.reshape(va_x,(-1,dim_scale))
        tr_y=np.reshape(tr_y,(-1,len(classes)))
        te_y=np.reshape(te_y,(-1,len(classes)))
        va_y=np.reshape(va_y,(-1,len(classes)))
